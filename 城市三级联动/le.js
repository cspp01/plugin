...
//省
        var $provinces=document.createElement('select');
        $provinces.setAttribute('name','provinces');
//一级城市
        var $citys=document.createElement('select');
        $citys.setAttribute('name','citys');
//县级市或县
        var $areas=document.createElement('select');
        $areas.setAttribute('name','areas');
        ...
        var provinceStr='';
//初始化
        for(var i=0;i<cityLevel['0'].length;i++){
            provinceStr+='<option value="'+cityLevel['0'][i]+'">'+cityLevel['0'][i]+'</option>';
        }
        $provinces.innerHTML='<option value="0">省份</option>'+provinceStr;
        $citys.innerHTML='<option value="00">地级市</option>';
        $areas.innerHTML='<option value="000">县或县级市</option>';

        $citySelect.appendChild($provinces);
        $citySelect.appendChild($citys);
        $citySelect.appendChild($areas);

//当值发生变化产生change事件
        var cityNum=null;
        $provinces.onchange=function(){
            str($provinces.value,'0','0',$citys);
        };
        $citys.onchange=function(){
            str($citys.value,cityNum,cityNum,$areas)
        };
        function str(a,b,c,d){
            var str='';
            var num=0;
            citysValue=a;
            str+= b=='0'&&c=='0'?'<option value="00">地级市</option>':'<option value="000">县或县级市</option>';
            if(citysValue&&citysValue!='00'){
                for(var i=0;i<cityLevel[b].length;i++) {
                    if(citysValue==cityLevel[b][i]){
                        num=i;
                        break;
                    }
                }
                for(var j=0;j<cityLevel[c+'_'+num].length;j++){
                    str+='<option value="'+cityLevel[c+'_'+num][j]+'">'+cityLevel[c+'_'+num][j]+'</option>';
                }
            }
            d.innerHTML=str;
            if(b=='0'&&c=='0') {
